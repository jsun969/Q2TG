...
(async () => {
  ...
  const forwardTgMessage = async (msg: TelegramBot.Message) => {
    try {
      ...
      //           const tipMsg = await tg.sendMessage(
      //             msg.chat.id,
      ...
      //     } catch (e) {
      //       const tipMsg = await tg.sendMessage(msg.chat.id, 'Bot 目前无法撤回其他用户的消息，Bot 需要「删除消息」权限', {
      //         disable_notification: true,
      //       });
      //       setTimeout(() => tg.deleteMessage(msg.chat.id, String(tipMsg.message_id)), 5000);
      //     }
  ...
  //           const tipMsg = await tg.sendMessage(msg.chat.id, '撤回 QQ 中对应的消息失败，QQ Bot 需要是管理员', {
  //             disable_notification: true,
  //           });
  //           setTimeout(() => tg.deleteMessage(msg.chat.id, String(tipMsg.message_id)), 5000);
  //         }
